...
- 마이크로소프트 프롬프트 엔지니어링 기법

# 한글 요약

```
1. 기본 원리
작동 방식: GPT 모델은 질문에 답하는 것이 아니라, 이전 텍스트 다음에 올 가장 그럴듯한 단어를 확률적으로 예측하여 문장을 완성하는 방식입니다.

핵심: 프롬프트 작성은 과학보다 '예술'에 가까우며, 경험과 직관이 필요합니다.

2. 프롬프트의 5가지 구성 요소
프롬프트를 짤 때 다음 요소들을 조합하면 더 좋은 결과를 얻을 수 있습니다.

지시 (Instructions): 모델이 수행해야 할 작업 (예: "이 글을 요약해", "프랑스어로 번역해").

주요 콘텐츠 (Primary Content): 모델이 처리해야 할 텍스트 (예: 번역할 문장, 요약할 뉴스 기사).

예시 (Examples): 가장 중요합니다. 원하는 입력과 출력의 예시를 보여주는 것(Few-shot learning)이 아무 예시 없이 질문하는 것(Zero-shot)보다 훨씬 정확합니다.

신호 (Cue): 모델이 답변을 시작할 방향을 잡아주는 유도 문구 (예: "핵심 포인트: • "라고 적어두면 불렛 포인트로 작성하기 시작함).

보조 콘텐츠 (Supporting Content): 결과물에 영향을 주는 배경 정보 (예: 사용자 취향, 현재 날짜 등).

3. 베스트 프랙티스 (잘 쓰는 꿀팁)
구체적으로 (Be Specific): 해석의 여지를 남기지 말고 범위를 좁히세요.

반복 강조 (Double Down): 중요한 지시는 텍스트의 앞과 뒤에 반복해서 배치하세요.

순서가 중요함 (Order Matters): 모델은 뒷부분에 있는 내용을 더 잘 기억하는 경향(최신 편향)이 있습니다. 지시사항의 배치 순서를 바꿔보세요.

빠져나갈 구멍 주기 (Give an "out"): 정보가 없으면 "찾을 수 없음"이라고 답하게 하여, 거짓말(환각)을 방지하세요.

4. 공간(토큰) 효율성
입력할 수 있는 양(토큰)에는 한계가 있습니다.

표(Table) 형식은 JSON보다 토큰을 덜 차지하면서도 모델이 잘 이해하는 효율적인 데이터 형식입니다.

불필요한 공백이나 구두점은 줄이는 것이 좋습니다.
```
